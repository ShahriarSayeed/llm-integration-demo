...
import { OpportunityMapCanvas } from "./OpportunityMapCanvas";
import { CustomerSearchDrawer } from "./CustomerSearchDrawer";
        ...
        if (config.showListings) {
          promises.push(
            supabase.functions.invoke("fetch-pool-listings", {
              ...
              const listings = ((data as { listings?: unknown[] })?.listings ?? []).map((l: Record<string, unknown>) => {
        ...
        if (config.showPermits) {
          promises.push(
            supabase.functions.invoke("fetch-pool-permits", {
              ...
              const permits = ((data as { permits?: unknown[] })?.permits ?? []).map((p: Record<string, unknown>) => ({
                id: String(p.id ?? ""), type: "permit" as const,
                name: String(p.description || p.address || ""), address: String(p.address ?? ""),
                lat: Number(p.lat), lng: Number(p.lng),
                metadata: { permitDate: p.permitDate, status: p.status, description: p.description, contractor: p.contractor, estimatedValue: p.estimatedValue, source: p.source },
              }));
        ...
        if (config.showProspects) {
          ...
          promises.push(
            supabase.functions.invoke("find-nearby-pool-prospects", {
              ...
              const prospects = ((data as { prospects?: unknown[] })?.prospects ?? []).map((p: Record<string, unknown>) => ({
